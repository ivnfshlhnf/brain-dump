<!--
SUPERSEDED by the Rolodex (docs/adr/0006-rolodex-replaces-field-notebook.md).

This is the Field Notebook: the app's design from its first build until the Rolodex replaced
it. The implementation is gone; this record is kept because the Rolodex inherits most of its
reasoning and cites it — the two type faces (serif for the user's thought, mono for the app's
scaffolding), the rule that a state always rides a word and never colour alone, and the
wet-to-dry countdown all carry over unchanged.

What did not carry over is the thing worth remembering: strict two-accent discipline, with
state encoded in the accents. The Rolodex spends colour on Category instead. If the grid ever
starts to look busy, this file is the argument for the other side of that trade.

The live design system is DESIGN.md at the repo root.
-->
