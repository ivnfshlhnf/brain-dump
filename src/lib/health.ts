// "Test connection" — the three external dependencies, checked independently.
//
...
/** Run all three checks, always. An earlier failure never short-circuits a later check:
 *  knowing that CouchDB is fine while the embedder is not is the entire point. */
export async function checkConnections(deps: HealthDeps): Promise<HealthReport> {
  ...
  log({
    level: report.couchdb.ok && report.chat.ok && report.embeddings.ok ? 'info' : 'error',
    op: 'health',
